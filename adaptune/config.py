...
with open(
    ...
    params["format"] = eval("alsa.PCM_FORMAT_" + params["format"])
        
    filter_params = config["filter_params"]
    
    domain = config["filter_domain"]
    
    base_filter = eval("pa.filters.Filter" + config["base_filter"])
    
    devices = config["devices"]
    
    
def dump_defaults(puts: bool = True, description: bool = False) -> Tuple[str, dict]:
    """
